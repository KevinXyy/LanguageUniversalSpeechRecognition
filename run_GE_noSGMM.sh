...
local/gp_data_prep.sh --config-dir=$PWD/conf --corpus-dir=$GP_CORPUS --languages="$GP_LANGUAGES" || exit 2;
...
for L in $GP_LANGUAGES; do
  mkdir -p exp/$L/mono;
  steps/train_mono.sh --nj 6 --cmd "$train_cmd" \
...
for L in $GP_LANGUAGES; do
  for lm_suffix in tgpr_sri; do
    (
      ...
      steps/decode.sh --nj 6 --cmd "$decode_cmd" $graph_dir data/$L/dev \
         exp/$L/mono/decode_dev_${lm_suffix}
      steps/decode.sh --nj 6 --cmd "$decode_cmd" $graph_dir data/$L/eval \
...
for L in $GP_LANGUAGES; do
 for lm_suffix in tgpr_sri; do
  (
    ...
    steps/decode_sgmm2.sh --use-fmllr true --nj 6 --cmd "$decode_cmd" \
...
for L in $GP_LANGUAGES; do
 for lm_suffix in tgpr_sri; do
  (
    ...
    steps/align_sgmm2.sh --nj 6 --cmd "$train_cmd" \
    ...
    steps/make_denlats_sgmm2.sh --nj 6 --sub-split 10 --cmd "$decode_cmd" \
...
#     num_states=$(grep "^$L" conf/sgmm.conf | cut -f2)
#     num_substates=$(grep "^$L" conf/sgmm.conf | cut -f3)
#     num_gauss=$(grep "^$L" conf/sgmm.conf | cut -f3)
#     mkdir -p exp/$L/sgmm2a
#     steps/train_sgmm2.sh --cmd "$train_cmd"  --spk-dim 0 --spk_dep_weights false \
...
# 
#         steps/decode_sgmm2.sh --nj 6 --cmd "$decode_cmd" $graph_dir data/$L/dev \
#           exp/$L/$sgmm/decode_dev_${lm_suffix}
#         steps/decode_sgmm2.sh --nj 6 --cmd "$decode_cmd" $graph_dir data/$L/eval \
#           exp/$L/$sgmm/decode_eval_${lm_suffix}
#       ) &
#     done  # loop over LMs
