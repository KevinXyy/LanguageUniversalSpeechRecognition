...
for L in $LANGUAGES; do
  ...
  test=data/$multi_L/$L/lang_test_tg
  if $filter_vocab_sri; then  # use SRILM to change LM vocab
    utils/format_lm_sri.sh --srilm-opts "$srilm_opts" \
      data/$multi_L/lang $lm data/$multi_L/local/dict/lexicon.txt "${test}_sri"
  
  else  # just remove out-of-lexicon words without renormalizing the LM
    utils/format_lm.sh data/$multi_L/lang $lm data/$multi_L/local/dict/lexicon.txt "$test"
  fi
  
  # Create a pruned version of the LM for building the decoding graphs, using 
  # 'prune-lm' from IRSTLM:
  mkdir -p data/$multi_L/local/lm
  prune-lm --threshold=1e-7 $lm /dev/stdout | gzip -c \
    > data/$multi_L/local/lm/${L}.tgpr.arpa.gz
  lm=data/$multi_L/local/lm/${L}.tgpr.arpa.gz
  test=data/$multi_L/$L/lang_test_tgpr
  if $filter_vocab_sri; then  # use SRILM to change LM vocab
    utils/format_lm_sri.sh data/$multi_L/lang $lm data/$multi_L/local/dict/lexicon.txt \
      "${test}_sri"
  else  # just remove out-of-lexicon words without renormalizing the LM
    utils/format_lm.sh data/$multi_L/lang $lm data/$multi_L/local/dict/lexicon.txt "$test"
  fi
