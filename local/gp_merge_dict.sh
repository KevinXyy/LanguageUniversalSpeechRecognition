...
mkdir -p data/$multi_L/local/dict
rm data/$multi_L/local/dict/lexicon_nosil.txt || true
for L in $LANGUAGES; do
  ...
  if [ ! -z "$map_dir" ]; then  # map the phones to a different phoneset
    if [ -f "$map_dir/$full_name" ]; then  # found the mapping file
      local/gp_norm_dict_${L}.pl -l -i "$pron_lex" -m "$map_dir/$full_name" \
	    | sort -u >> data/$multi_L/local/dict/lexicon_nosil.txt 
    else
      echo "No phone mapping '$map_dir/$full_name': keeping original phoneset";
      local/gp_norm_dict_${L}.pl -l -i "$pron_lex" | sort -u \
	>> data/$multi_L/local/dict/lexicon_nosil.txt
    fi
  else
    local/gp_norm_dict_${L}.pl -l -i "$pron_lex" | sort -u \
      >> data/$multi_L/local/dict/lexicon_nosil.txt
  ...
  (printf '!SIL\tsil\n<unk>\tspn\n';) \
    | cat - data/$multi_L/local/dict/lexicon_nosil.txt \
    > data/$multi_L/local/dict/lexicon.txt;
  echo "Done"
done
wait;


printf "Language - ${LANGUAGES}: extracting phone lists ... "
# silence phones, one per line.
{ echo sil; echo spn; } > data/$multi_L/local/dict/silence_phones.txt
echo sil > data/$multi_L/local/dict/optional_silence.txt
cut -f2- data/$multi_L/local/dict/lexicon_nosil.txt | tr ' ' '\n' | sort -u \
  > data/$multi_L/local/dict/nonsilence_phones.txt
...
( tr '\n' ' ' < data/$multi_L/local/dict/silence_phones.txt; echo;
  tr '\n' ' ' < data/$multi_L/local/dict/nonsilence_phones.txt; echo;
  ) > data/$multi_L/local/dict/extra_questions.txt
echo "Done"
