...
# . local.conf || exit 1;
lang_list=(CZ FR GE PL PO RU SP SW)
num_langs=${#lang_list[@]}
...
for lang_index in `seq 0 $[$num_langs-1]`; do
  ...
  multi_data_dirs[$lang_index]=data/${lang_list[$lang_index]}/train
  utils/fix_data_dir.sh data/${lang_list[$lang_index]}/train
  utils/fix_data_dir.sh data/${lang_list[$lang_index]}/dev
  utils/fix_data_dir.sh data/${lang_list[$lang_index]}/eval
  multi_egs_dirs[$lang_index]=exp/${lang_list[$lang_index]}/nnet3mono/tdnn1a/egs_hr
  echo ${multi_egs_dirs[$lang_index]}
  rm -rf ${multi_egs_dirs[$lang_index]}
  multi_ali_dirs[$lang_index]=exp/${lang_list[$lang_index]}/${alidir}
...
if [ $stage -le 8 ]; then
  ...
  relu-renorm-layer name=tdnn1 input=Append(input@-2,input@-1,input,input@1,input@2$ivector_to_append) dim=650
  relu-renorm-layer name=tdnn2 dim=650
  relu-renorm-layer name=tdnn3 input=Append(-1,0,1) dim=650
  relu-renorm-layer name=tdnn4 input=Append(-1,0,1) dim=650
  relu-renorm-layer name=tdnn5 input=Append(-3,0,3) dim=650
  relu-renorm-layer name=tdnn6 input=Append(-6,-3,0) dim=650
...
EOF
  ...
  for lang_index in `seq 0 $[$num_langs-1]`;do
    ...
    echo " relu-renorm-layer name=prefinal-affine-lang-${lang_index} input=tdnn_bn dim=650"
...
if [ $stage -le 9 ]; then
  ...
  local/nnet3/prepare_multilingual_egs.sh --num-jobs 8 --cmd "$decode_cmd" \
    "${ivector_opts[@]}" \
...
decode_lang_list=(CZ FR GE PL PO RU SP SW)
...
if [ $stage -le 13 ]; then
  ...
  for lang_index in `seq 0 $[$num_decode_lang-1]`; do
    if [ ! -f $dir/${decode_lang_list[$lang_index]}/decode_dev10h.pem/.done ]; then
      ...
      touch $dir/${decode_lang_list[$lang_index]}/decode_dev/.done
      touch $dir/${decode_lang_list[$lang_index]}/decode_eval/.done
    fi
  done
fi
