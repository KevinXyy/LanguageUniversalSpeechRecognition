...
##
####################################################################
if [ ! -f exp/$lang/mono/graph/HCLG.fst ];then
  utils/mkgraph.sh \
    data/$lang/lang_test_tgpr_sri exp/$lang/mono exp/$lang/mono/graph |tee exp/$lang/mono/mkgraph.log
fi
...
if [ -f $nnet3_dir/$lang/final.mdl ]; then
  ...
  for unseen in $dir; do
    decode=$nnet3_dir/$lang/decode_$unseen
    rm -f $decode/.done
    if [ ! -f $decode/.done ]; then
      mkdir -p $decode
      score_opts="--skip-scoring false"
      [ ! -z $iter ] && iter_opt="--iter $iter"
      nj=$(wc -l <data/$lang/$unseen/spk2utt)
      steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" $iter_opt \
            --stage $decode_stage \
            exp/$lang/mono/graph $data/$unseen $decode | tee $decode/decode.log
