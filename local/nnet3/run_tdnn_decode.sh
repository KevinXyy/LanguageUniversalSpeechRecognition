...
export GP_LANGUAGES="CZ PL PO RU SP SW"   #"CZ GE PL PO RU SP SW"
for LAN in $GP_LANGUAGES; do
  echo "123"
  stage=0
  gmm_dir=exp/$LAN/${gmm}
  ...
  if [ $stage -le 14 ]; then
    ...
    for data in $test_sets; do
      (
        ...
        nj=$(wc -l <data/$LAN/${data}/spk2utt)
        for lmtype in tgpr_sri; do
          graph_dir=exp/$LAN/nnet3/tdnn1a/graph_${lmtype}
          mkdir -p $graph_dir
          utils/mkgraph.sh data/$LAN/lang_test_${lmtype} exp/$LAN/nnet3/tdnn1a \
          $graph_dir
          steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd"  --num-threads 4 \
            ${graph_dir} data/$LAN/${data} ${dir}/decode_${lmtype}_${data_affix} || exit 1
        done

        steps/lmrescore.sh --cmd "$decode_cmd" data/$LAN/lang_test_{tgpr,tg}_sri \
          data/$LAN/${data} ${dir}/decode_{tgpr_sri,tg_sri}_${data_affix} || exit 1
        # steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" \
        #   data/$LAN/lang_test_bd_{tgpr,fgconst} \
        #  data/$LAN/${data} ${dir}/decode_${lmtype}_${data_affix}{,_fg} || exit 1
      ) || touch $dir/.error &
    done
    wait
